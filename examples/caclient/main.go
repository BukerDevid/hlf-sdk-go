...
func main() {
	...
	log.Println(caClient.Enroll(ctx, name, `123321`, &x509.CertificateRequest{
		...
		}{Country: []string{`RU`}, Organization: []string{`S7`}, OrganizationalUnit: []string{`ORG`}, Locality: []string{`Moscow`}, Province: []string{`Moscow`}, StreetAddress: []string{`Pushkin 7`}, PostalCode: []string{`100001`}, CommonName: name},
